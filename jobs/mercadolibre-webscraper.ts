...
class MercadoLibreScrapper extends Job {
    ...
    async run() {
        if (!this.isReady || this.isWorking) {
            console.log("Scrapper not ready");
            return;
        }
        ...
        let selected_category = this.categories[0];
                    ...
                    .query('SELECT id FROM meli_header WHERE url = ?', [url]).then(([rows, fields]) => {
                        (async () => {
                            ...
                            if (rows.length == 0) {
                                ...
                                if (rows.length > 1) console.log(`WARNING: More than 1 row found for '${id}'`);
                            ...
                            /*await this.dbpool.promise()
                                .query(
                                    `SELECT id FROM meli_items WHERE id = ? AND created_at > NOW() - INTERVAL ? MINUTE`,
                                    [id, jobs.mercadolibre.config.minTimeDifference]
                                )
                                .then(([rows, fields]) => {
                                    rows = rows as any[];
